...
_IMAGE_ACCOUNT_UUID_NAMESPACE = UUID("9f57d9f0-4b6a-4d4b-9f24-5f26f2fb1de8")
_MANAGED_IMAGE_RESULT_IDEMPOTENCY_FIELD = "managed_image_result_idempotency_keys"
_MANAGED_IMAGE_RESULT_IDEMPOTENCY_LIMIT = 256
_MANAGED_IMAGE_RESULT_IDEMPOTENCY_KEY_MAX_LENGTH = 256
...
class ImageAccountSelectionError(RuntimeError):
    ...
    # deadline_exceeded is request-scoped and never changes a persisted
    # Upstream Account state.  Preflight failures retain their transport or
    # authentication category so the caller can apply the same account-switch
    # policy as it would for a failure during generation.
    DEFAULTS: dict[str, tuple[int, str, str]] = {
        ...
        "auth_invalid": (401, "authentication_error", "auth_invalid"),
        "upstream_unavailable": (502, "server_error", "upstream_unavailable"),
        "upstream_connection_failed": (502, "server_error", "upstream_connection_failed"),
        "upstream_connection_timeout": (504, "server_error", "upstream_connection_timeout"),
        "upstream_challenge_required": (503, "server_error", "upstream_challenge_required"),
    }
...
class AccountService:
    ...
    _OAUTH_USER_AGENT = CHROME146_USER_AGENT
    ...
    @classmethod
    def image_account_id(cls, account_id: object) -> UUID | None:
        """Return the durable queue UUID for an account management ID.

        The account pool historically exposes ``management_id`` strings while
        the persistent image queue stores UUIDs.  Deriving the UUID from the
        stable management ID keeps queue leases valid when an access token is
        rotated.
        """
        ...
        if raw.startswith("acct_"):
            suffix = raw[5:]
            if len(suffix) != 24 or not all(
                char in "0123456789abcdef" for char in suffix
            ):
                return None
            return uuid5(_IMAGE_ACCOUNT_UUID_NAMESPACE, raw)
        try:
            return UUID(raw)
        except (ValueError, AttributeError):
            return None

    def _resolve_image_account_locked(
        self,
        account_id: object,
    ) -> tuple[str, dict] | None:
        raw = str(account_id or "").strip().lower()
        if not raw:
            return None
        requested_uuid = self.image_account_id(raw)
        for access_token, account in self._accounts.items():
            management_id = str(account.get("management_id") or "").strip().lower()
            if not management_id:
                continue
            if raw != management_id and (
                requested_uuid is None
                or requested_uuid != self.image_account_id(management_id)
            ):
                continue
            return access_token, dict(account)
        return None
    ...
    @classmethod
    def _normalize_managed_image_result_idempotency_keys(
        cls,
        value: object,
    ) -> list[str]:
        ...
        seen: set[str] = set()
        for item in value:
            key = str(item or "").strip()[:_MANAGED_IMAGE_RESULT_IDEMPOTENCY_KEY_MAX_LENGTH]
            if not key or key in seen:
                continue
            seen.add(key)
            keys.append(key)
        return keys[-_MANAGED_IMAGE_RESULT_IDEMPOTENCY_LIMIT:]
    ...
    def _normalize_account(self, item: dict) -> dict | None:
        ...
        if _MANAGED_IMAGE_RESULT_IDEMPOTENCY_FIELD in normalized:
            normalized[_MANAGED_IMAGE_RESULT_IDEMPOTENCY_FIELD] = (
                self._normalize_managed_image_result_idempotency_keys(
                    normalized.get(_MANAGED_IMAGE_RESULT_IDEMPOTENCY_FIELD),
                )
            )
        ...
        normalized["success"] = self._quota_value(normalized.get("success"), 0)
        normalized["fail"] = self._quota_value(normalized.get("fail"), 0)
        normalized["invalid_count"] = self._quota_value(normalized.get("invalid_count"), 0)
    ...
    def get_available_access_token(
        ...
        preflight_failure_kinds: set[str] = set()
        ...
        for _attempt in range(max_attempts):
            ...
            try:
                ...
                if failure.code in ImageAccountSelectionError.DEFAULTS:
                    preflight_failure_kinds.add(failure.code)
                self.release_image_slot(access_token)
                if deadline_monotonic is not None and time.monotonic() >= deadline_monotonic:
                    raise ImageAccountSelectionError(
                        "deadline_exceeded",
        ...
        if len(preflight_failure_kinds) == 1:
            kind = next(iter(preflight_failure_kinds))
            raise ImageAccountSelectionError(
                kind,
                f"all candidate image accounts failed remote preflight: {kind}",
            )
        raise ImageAccountSelectionError(
            "unavailable",
    ...
    def list_image_account_candidates(self) -> list[Any]:
        ...
        with self._image_slot_condition:
            candidates: list[ImageAccountCandidate] = []
            for account in self._accounts.values():
                if not self._is_image_account_available(account):
                    continue
                access_token = str(account.get("access_token") or "").strip()
                management_id = str(account.get("management_id") or "").strip()
                account_id = self.image_account_id(management_id)
    ...
    def prepare_image_account(self, account_id: object) -> str:
        """Resolve and refresh a durable queue account before generation."""
        self._refresh_accounts_snapshot_if_stale(wait_for_refresh=True)
        with self._image_slot_condition:
            resolved = self._resolve_image_account_locked(account_id)
            if resolved is None:
                raise ImageAccountSelectionError(
                    "unavailable",
                    "persistent image account no longer exists",
                )
            access_token, account = resolved
            if not self._is_image_account_available(account):
                raise ImageAccountSelectionError(
                    "unavailable",
                    "persistent image account is not currently available",
                )
        return self.ensure_access_token(
            access_token,
            event="image_queue_prepare_account",
            image_scope=True,
            raise_on_error=True,
        )
    ...
    def _pool_health_metrics_from_accounts(
        ...
        *,
        ...
        return {
            "current_quota": sum(
                cls._quota_value(item.get("quota"), 0)
    ...
    def _record_image_result(
        ...
        *,
        ...
        normalized_idempotency_key = str(idempotency_key or "").strip()[
            :_MANAGED_IMAGE_RESULT_IDEMPOTENCY_KEY_MAX_LENGTH
        ]
        with self._image_slot_condition:
            access_token = self._resolve_access_token_locked(access_token)
            try:
                ...
                existing_idempotency_keys = (
                    self._normalize_managed_image_result_idempotency_keys(
                        current.get(_MANAGED_IMAGE_RESULT_IDEMPOTENCY_FIELD),
                    )
                )
                if (
                    normalized_idempotency_key
                    and normalized_idempotency_key in existing_idempotency_keys
                ):
                    result = dict(current)
                    return result
                expected_generation = None
                ...
                if normalized_idempotency_key:
                    next_item[_MANAGED_IMAGE_RESULT_IDEMPOTENCY_FIELD] = [
                        *existing_idempotency_keys,
                        normalized_idempotency_key,
                    ][-_MANAGED_IMAGE_RESULT_IDEMPOTENCY_LIMIT:]
    ...
    def mark_image_result(
        ...
        *,
        ...
        _ = capabilities
        return self._record_image_result(
            access_token,
            success,
            failure=failure,
    ...
    def record_managed_image_result(
        ...
        *,
        ...
        """Record persistent-worker accounting without touching request slots."""
        _ = error
        self._refresh_accounts_snapshot_if_stale(wait_for_refresh=True)
        with self._image_slot_condition:
            resolved = self._resolve_image_account_locked(account_id)
            if resolved is None:
                return None
            access_token, _account = resolved
        return self._record_image_result(
            access_token,
            success,
            failure=failure,
