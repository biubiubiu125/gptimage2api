...
def _is_queue_private_artifact_path(path: str) -> bool:
    try:
        parts = PurePosixPath(normalize_image_relative_path(path)).parts
    except HTTPException:
        return False
    if len(parts) == 3 and parts[1] in {"input", "mask"}:
        try:
            UUID(parts[0])
...
class WebDAVClient:
    ...
    def put_atomic(self, rel: str, payload: bytes, content_type: str = "image/png") -> str:
        ...
        return self.put(rel, payload, content_type=content_type)
    ...
    def test(self) -> dict[str, object]:
        ...
        test_rel = ".gptimage2api_webdav_test.txt"
        try:
            self.put(test_rel, b"gptimage2api webdav test\n", content_type="text/plain")
...
class ImageStorageService:
    ...
    def _index_file_lock_path(self) -> Path:
        return self._index_file_lock

    def _private_webdav_settings(self) -> dict[str, object]:
        # The upstream storage catalog has no second private WebDAV target.
        # Queue staging artifacts therefore remain local and are cleaned by
        # ArtifactService without entering the public gallery catalog.
        return {}
    ...
    def _load_clean_index(self) -> dict[str, dict[str, object]]:
        ...
        for rel, item in items.items():
            if not _is_image_rel(rel) or _is_queue_private_artifact_path(rel):
                continue
    ...
    def save_at_path(
        ...
        previous_local_payload: bytes | None = None
        with self._item_guard(rel):
            with self._index_guard():
                previous_item = self._load_clean_index().get(rel)
            if path.is_file():
                try:
                    previous_local_payload = path.read_bytes()
                except OSError:
                    previous_local_payload = None
            item: dict[str, object] = {}
            try:
                if stored_local:
                    path.parent.mkdir(parents=True, exist_ok=True)
                ...
                if stored_webdav:
                    ...
                    with _webdav_client(self.settings()) as client:
                        remote_url = client.put(rel, image_data, content_type=content_type_for_path(rel))
                dimensions = _image_dimensions(image_data)
                item = {
                    "rel": rel,
                    "path": rel,
                ...
                if local_written:
                    try:
                        if previous_local_payload is not None:
                            path.parent.mkdir(parents=True, exist_ok=True)
                            path.write_bytes(previous_local_payload)
                        elif path.exists():
                            path.unlink()
                    except OSError:
                        pass
                if remote_upload_attempted and previous_item is None:
                    self._cleanup_failed_remote_publish(
                        rel,
                        _clean(item.get("generation")),
                    )
                raise

    def _cleanup_failed_remote_publish(self, rel: str, generation: str) -> None:
        ...
        try:
            ...
            # Keep trying the immediate delete even if the durable marker
            # cannot be written; the original index failure is still raised.
            pass
        ...
        try:
            with self._index_guard():
                pending = self._load_remote_delete_pending()
                if self._tombstone_matches(pending.get(rel), tombstone):
                    pending.pop(rel, None)
                    self._save_remote_delete_pending(pending)
        except Exception:
            pass
    ...
    def existing_paths(self, rels: list[str]) -> set[str]:
        safe_rels = list(dict.fromkeys(
            ...
            and not _is_queue_private_artifact_path(safe_rel)
        ))
        existing = {
            safe_rel
        ...
        with self._index_guard():
            ...
            if refresh_index:
                for path in root.rglob("*"):
                    ...
                    if _is_queue_private_artifact_path(rel):
                        continue
                    if rel in indexed:
                        continue
                    dimensions = None
    ...
    def compress_local_images(
        ...
        *,
        ...
        for rel in exclude_rels or set():
            try:
                excluded.add(normalize_image_relative_path(rel))
            except HTTPException:
                continue
        ...
        for path in sorted(image_root.rglob("*.png")):
            ...
            if rel in excluded or _is_queue_private_artifact_path(rel):
                continue
