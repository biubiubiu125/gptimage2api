...
DEFAULT_IMAGE_STORAGE = {
    ...
    "webdav_username": "",
    "webdav_password": "",
    "webdav_root_path": "gptimage2api/images",
    "public_base_url": "",
}
...
DEFAULT_PROXY_RUNTIME_USER_AGENT = CHROME146_USER_AGENT
...
def _normalize_proxy_runtime_settings(value: object) -> dict[str, object]:
    ...
    normalized_clearance.pop("flaresolverr_url", None)
...
def _validate_image_storage_settings(settings: dict[str, object]) -> None:
    if not _normalize_bool(settings.get("enabled"), False):
        ...
        enabled = True
    public_base_url = str(settings.get("public_base_url") or "").strip()
    if public_base_url:
        try:
            validate_public_image_base_url(public_base_url)
        except Exception as exc:
            raise ValueError("公开访问前缀必须是有效的 HTTP(S) 地址，路径只能为空或 /images") from exc
    if not enabled:
        return
...
class ConfigStore:
    ...
    def auth_key(self) -> str:
        ...
        return _normalize_auth_key(
            env_value("GPTIMAGE2API_AUTH_KEY", "CHATGPT2API_AUTH_KEY")
            or bootstrap.get("auth-key")
        )
    ...
    def images_dir(self) -> Path:
        configured_root = str(
            env_value(
                "GPTIMAGE2API_IMAGE_QUEUE_ARTIFACT_ROOT",
                "CHATGPT2API_IMAGE_QUEUE_ARTIFACT_ROOT",
            )
            or os.getenv("IMAGE_QUEUE_ARTIFACT_ROOT")
            or ""
        ).strip()
        path = Path(configured_root).expanduser() if configured_root else DATA_DIR / "images"
    ...
    def update(self, data: dict[str, object]) -> dict[str, object]:
        with self._lock:
            ...
            if "runtime_capacity" in updates:
                updates["runtime_capacity"] = _normalize_runtime_capacity_settings(
                    updates.get("runtime_capacity")
                )
            if "quota_limits" in updates:
