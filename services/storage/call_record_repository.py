...
class CallRecordRepository:
    ...
    def find_by_call_id(self, call_id: object) -> dict[str, Any] | None:
        """Find the durable outer record for a LoggedCall correlation id.

        ``call_id`` lives inside the sanitized payload detail, while the
        database primary key is an independent record id.  Keep this lookup
        dialect-neutral because the application database supports both SQLite
        and PostgreSQL and the payload column is JSON/JSONB on those backends.
        """
        target = self._clean(call_id)
        if not target:
            return None
        session = self.Session()
        try:
            rows = session.scalars(
                select(CallRecordModel)
                .where(CallRecordModel.type == "call")
                .order_by(CallRecordModel.sequence.desc())
            )
            for row in rows:
                payload = self._row_payload(row)
                detail = payload.get("detail")
                if isinstance(detail, dict) and self._clean(detail.get("call_id")) == target:
                    return payload
            return None
        finally:
            session.close()

    def find_by_task_id(self, task_id: object) -> list[dict[str, Any]]:
        """Find all call records attached to one durable image task."""
        target = self._clean(task_id)
        if not target:
            return []
        session = self.Session()
        try:
            ...
            for row in rows:
                payload = self._row_payload(row)
                detail = payload.get("detail")
                if isinstance(detail, dict) and self._clean(detail.get("task_id")) == target:
                    records.append(payload)
            return records
        finally:
            session.close()

    def update(self, record_id: str, record: CallRecordWrite) -> dict[str, Any] | None:
        """Replace a call record in place and invalidate incremental cursors."""
        target = self._clean(record_id)
        if not target:
            return None
        payload = self._copy(record.payload)
        payload["id"] = target
        ...
        try:
            row = session.scalar(
                select(CallRecordModel)
                .where(CallRecordModel.id == target)
                .with_for_update()
            )
            if row is None:
                return None
            changed = (
                self._row_payload(row) != payload
                or row.event_time != event_time
                or row.type != self._clean(payload.get("type"))
                or row.outcome != self._clean(record.outcome)
                or row.endpoint != self._clean(record.endpoint)
                or row.model != self._clean(record.model)
                or row.account_email != self._clean(record.account_email)
                or row.conversation_id != self._clean(record.conversation_id)
                or row.business_kind != self._clean(record.business_kind)
                or row.search_text != self._clean(record.search_text).lower()
            )
            if changed:
                row.event_time = event_time
                row.event_day = event_time[:10]
                row.type = self._clean(payload.get("type"))
                row.outcome = self._clean(record.outcome)
                row.endpoint = self._clean(record.endpoint)
                row.model = self._clean(record.model)
                row.account_email = self._clean(record.account_email)
                row.conversation_id = self._clean(record.conversation_id)
                row.business_kind = self._clean(record.business_kind)
                row.search_text = self._clean(record.search_text).lower()
                row.payload = payload
