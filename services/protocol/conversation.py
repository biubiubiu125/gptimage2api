...
def _check_cancellation_callback(callback: Callable[[], None] | None) -> None:
    if callable(callback):
        callback()
...
def _download_image_bytes_with_monitor(
    ...
    try:
        if callable(request.cancel_requested_callback):
            downloaded_images = backend.download_image_bytes(
                image_urls,
                cancel_callback=request.cancel_requested_callback,
            )
        else:
...
class ImageOutput:
    ...
    def to_chunk(self) -> dict[str, Any]:
        ...
        if self.call_status:
            chunk["_call_status"] = self.call_status
        if self.image_urls:
...
def _get_detailed_failure_from_tasks(
    ...
    _check_cancellation_callback(cancel_callback)
    try:
        if wait_secs > 0:
            deadline_timeout = getattr(backend, "_deadline_timeout", None)
            _time.sleep(
                deadline_timeout(wait_secs)
                if callable(deadline_timeout)
                else wait_secs
            )
    except Exception as exc:
    ...
    for task in tasks:
        ...
        if candidate is None:
            task_facts = extract_message_facts(task)
            candidate = classify_message_facts(
                role=str(task_facts.get("role") or ""),
                content_type=str(task_facts.get("content_type") or ""),
                status=str(task_facts.get("status") or ""),
                end_turn=bool(task_facts.get("end_turn")),
                is_error=bool(task_facts.get("is_error")),
                blocked=bool(task_facts.get("blocked")),
                has_image_output=False,
                has_text=bool(task_facts.get("has_text")),
                turn_use_case=str(task_facts.get("turn_use_case") or ""),
                async_task_type=str(task_facts.get("async_task_type") or ""),
                message_type=str(task_facts.get("message_type") or ""),
                codes=task_facts.get("codes") or (),
                raw_detail=str(
                    task.get("error")
...
def stream_image_outputs(
    ...
    def persist_remote_checkpoint(event: Mapping[str, Any]) -> None:
        ...
        event_file_ids = tuple(str(item) for item in event.get("file_ids") or [])
        event_sediment_ids = tuple(str(item) for item in event.get("sediment_ids") or [])
        checkpoint_key = (
        ...
        request.checkpoint_callback(JobCheckpoint(
            ...
            file_ids=list(event_file_ids),
            sediment_ids=list(event_sediment_ids),
        ))
    ...
    if not file_ids and not sediment_ids and last.get("blocked"):
        _task_failure, detailed_error = _get_detailed_failure_from_tasks(
            backend,
            conversation_id,
            cancel_callback=request.cancel_requested_callback,
        )
...
def stream_codex_image_outputs(
    ...
    for event in backend.iter_codex_image_response_events(
        ...
        images.extend(_codex_response_images(event))
        _check_image_cancellation(request)
...
def _image_account_lookup_log_data(
        token: str,
        *,
        call_id: str = "",
        account_email: str = "",
        account_found: bool = False,
        account_wait_ms: int = 0,
        index: int = 0,
) -> dict[str, Any]:
    token_text = str(token or "").strip()
    fingerprint = hashlib.sha256(token_text.encode("utf-8")).hexdigest()[:24] if token_text else ""
    return {
        ...
        "account_wait_ms": int(account_wait_ms),
        "index": int(index),
    }
...
def _generate_single_image(
    ...
    while True:
        ...
        logger.debug({
            **_image_account_lookup_log_data(
                token,
                call_id=request.call_id,
                account_email=account_email,
                account_found=bool(account),
...
def stream_image_outputs_with_pool(request: ConversationRequest) -> Iterator[ImageOutput]:
    ...
    公共生图请求必须先进入 Image Tasks 持久化队列；这里不再创建
    请求级线程池，避免绕过队列的租约、取消和断点恢复语义。
    """
    _check_image_cancellation(request)
    if not request.managed_access_token:
        _monitor_image_stage(
...
def _image_stream_payload(output: ImageOutput, event_type: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if output.call_status:
        item["_call_status"] = output.call_status
...
def stream_image_chunks(
    ...
    for output in outputs:
        if output.kind == "progress":
            progress_type = str(output.upstream_event_type or "in_progress").strip()
            event_type = (
                progress_type
                if progress_type.startswith(f"{prefix}.")
                else f"{prefix}.{progress_type}"
            )
            progress_payload: dict[str, Any] = {}
            if output.task_id:
                progress_payload["task_id"] = output.task_id
            progress_payload.update({
                str(key): value
                for key, value in output.progress_fields.items()
                if str(key).strip()
            })
            if output.text:
                progress_payload["message"] = output.text
            if progress_payload:
                yield _image_stream_payload(output, event_type, progress_payload)
