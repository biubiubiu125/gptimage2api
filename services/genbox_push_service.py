...
_auto_push_runner = BoundedTaskRunner(
    ...
    max_workers=env_int("GPTIMAGE2API_GENBOX_PUSH_CONCURRENCY", 8),
    queue_size=env_int("GPTIMAGE2API_GENBOX_PUSH_QUEUE_SIZE", 64),
)
...
def _rel_from_stored_url(url: str, *, base_url: str | None = None) -> str | None:
    ...
    for base in (
        ...
        origin = normalize_url_origin(base)
        if origin is not None:
            configured_origins.add(origin)
    if normalize_url_origin(raw) not in configured_origins:
        return None
    path = parsed.path or ""
    if "/images/" in path:
