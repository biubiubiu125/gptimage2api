...
UPDATE_ARCHIVE_NAME = "gptimage2api-app.tar.gz"
...
class ReleaseBundleInstaller:
    ...
    def install(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix=".gptimage2api-update-", dir=self.root) as temp_value:
            ...
            bundle_dir = extract_dir / "gptimage2api-app"
    ...
    def _download(url: str, destination: Path, *, max_bytes: int) -> None:
        ...
        for _ in range(4):
            response = curl_requests.get(
                ...
                stream=True,
                **build_http_target_request_options(current_url),
...
class UpdateService:
    ...
    def _start_worker_thread(callback: Callable[[], None]) -> None:
        thread = threading.Thread(target=callback, name="gptimage2api-update", daemon=True)
        thread.start()
