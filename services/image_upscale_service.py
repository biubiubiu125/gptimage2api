...
def upscale_image_with_status(image_data: bytes, requested_size: object) -> UpscaleOutcome:
    if not image_data or not config.image_upscale_enabled:
        return UpscaleOutcome(image_data)
    ...
    try:
        if engine == "sharp_lanczos3":
            try:
                ...
                event_type = "upscale_engine_fallback"
                event_data = {
                    "from_engine": "sharp_lanczos3",
                    "to_engine": "pillow_lanczos",
        ...
        return UpscaleOutcome(result, event_type, event_data)
    except Exception as exc:
        error = diagnostic_excerpt(exc, 500)
        logger.warning({
...
def upscale_image_if_needed(image_data: bytes, requested_size: object) -> bytes:
    return upscale_image_with_status(image_data, requested_size).payload
