...
class RealtimeMonitorService:
    def __init__(self) -> None:
        completed_limit = env_int("GPTIMAGE2API_MONITOR_COMPLETED_LIMIT", 500, 50, 5000)
        event_limit = env_int("GPTIMAGE2API_MONITOR_EVENT_LIMIT", 1000, 100, 10000)
        self._lock = Lock()
        ...
        with self._lock:
            ...
            if record is None:
                completed_index = next(
                    (
                        index
                        for index in range(len(self._completed) - 1, -1, -1)
                        if self._completed[index].get("call_id") == call_id
                    ),
                    None,
                )
                if completed_index is not None:
                    # A durable image task may finish after the request timeout
                    # has already written a failed monitor record. Reopen that
                    # record instead of creating a duplicate or ignoring the
                    # authoritative terminal task result.
                    record = dict(self._completed[completed_index])
                    del self._completed[completed_index]
                else:
                    record = {
                        "call_id": call_id,
