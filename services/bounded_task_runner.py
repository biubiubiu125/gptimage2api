...
def env_int(name: str, default: int, *, minimum: int = 1) -> int:
    try:
        value = next(
            (
                int(str(os.getenv(candidate, "") or default).strip())
                for candidate in _env_candidates(name)
                if str(os.getenv(candidate, "") or "").strip()
            ),
            int(default),
        )
...
class BoundedTaskRunner:
    ...
    def submit_future(
        ...
        **kwargs: Any,
        ...
        """Submit work while exposing completion through a ``Future``.

        The runner remains responsible for admission and execution.  The
        future is only an observation handle, allowing result aggregation
        without falling back to an unbounded ``ThreadPoolExecutor``.
        """
