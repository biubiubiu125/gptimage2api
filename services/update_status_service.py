...
GITHUB_REPOSITORY = env_value(
    "GPTIMAGE2API_GITHUB_REPOSITORY",
    "CHATGPT2API_GITHUB_REPOSITORY",
).strip().strip("/")
...
GITHUB_CHANGELOG_URL = (
    f"https://api.github.com/repos/{GITHUB_REPOSITORY}/contents/CHANGELOG.md?ref=main"
    if GITHUB_REPOSITORY
    else ""
)
...
MANAGED_RUNTIME_MARKER = ".gptimage2api-image-version"
...
def _fetch_latest_release() -> Mapping[str, object]:
    if not GITHUB_LATEST_RELEASE_URL:
        raise UpdateCheckError("未配置 gptimage2api 发布仓库")
    response = curl_requests.get(
        ...
        stream=True,
        **build_http_target_request_options(GITHUB_LATEST_RELEASE_URL),
...
def _fetch_changelog() -> str:
    ...
    response = curl_requests.get(
        ...
        timeout=UPDATE_CHECK_TIMEOUT_SECS,
        stream=True,
        **build_http_target_request_options(GITHUB_CHANGELOG_URL),
        **proxy_settings.build_session_kwargs(
            resource=True,
            upstream=True,
            verify=not proxy_settings.should_skip_ssl_verify(),
...
def _build_type() -> str:
    configured = env_value("GPTIMAGE2API_BUILD_TYPE", "CHATGPT2API_BUILD_TYPE").lower()
