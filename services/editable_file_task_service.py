...
def _task_request_payload(
    ...
    return {
        ...
        "base64_images": [str(item) for item in base64_images if str(item or "").strip()],
        "base_url": str(base_url or ""),
        "identity": {
            "id": _clean(identity.get("id")),
...
def _parse_task_request(request: dict[str, Any]) -> tuple[str, list[str], dict[str, object], str]:
    prompt = str(request.get("prompt") or "")
    images = [
        str(item)
        for item in (request.get("base64_images") or [])
        if str(item or "").strip()
    ]
    raw_identity = request.get("identity") if isinstance(request.get("identity"), dict) else {}
    identity = {
        "id": _clean(raw_identity.get("id")),
        "name": _clean(raw_identity.get("name")),
        "role": _clean(raw_identity.get("role")),
    }
    base_url = str(request.get("base_url") or "")
    return prompt, images, identity, base_url


def _request_can_resume(kind: str, prompt: str, images: list[str]) -> bool:
    if kind == "psd":
        return bool(images)
    return bool(str(prompt or "").strip()) or kind == "ppt"
...
class EditableFileTaskService:
    ...
    def _submit(self, identity: dict[str, object], *, client_task_id: str, kind: str, prompt: str, base64_images: list[str], base_url: str) -> dict[str, Any]:
        ...
        now = _now_iso()
        request = _task_request_payload(kind, prompt, base64_images, identity, base_url)
        with self._lock:
            ts = time.time()
            ...
            if not created:
                ...
                if status in UNFINISHED_STATUSES and key in self._inflight:
                    return _public_task(task)
                task = self._repository.update(
                    owner,
                    task_id,
        ...
        self._start_task(key, kind, prompt, base64_images, dict(identity), base_url)
        return _public_task(task)
    ...
    def _start_task(
        ...
        threading.Thread(
            ...
            args=(key, kind, prompt, list(base64_images), dict(identity), base_url),
            name=f"{kind}-file-task-{key[-16:]}",
            daemon=True,
        ).start()
    ...
    def _recover_unfinished_locked(self) -> None:
        pending_starts: list[tuple[str, str, str, list[str], dict[str, object], str]] = []
        for task in self._repository.list_by_status(UNFINISHED_STATUSES):
            ...
            prompt, images, identity, base_url = _parse_task_request(request)
            if not owner_id or not task_id or not _request_can_resume(kind, prompt, images):
