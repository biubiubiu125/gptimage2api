...
from services.browser_fingerprint import (
    ...
    chrome146_client_contextual_info,
    chrome146_headers,
...
from services.returned_url_verifier import ReturnedUrlVerificationError, open_verified_returned_image_url
...
class CodexTransportError(ConnectionError):
    """Codex Responses 请求无法建立或维持上游连接。"""


class CodexTransportTimeoutError(TimeoutError):
    """Codex Responses 请求在连接或流式读取阶段超时。"""


class ImageOperationCancelled(RuntimeError):
    """Image task cancellation raised while an upstream operation is in flight."""


class UpstreamChallengeRequiredError(RuntimeError):
    """上游要求当前实现尚未提供的挑战凭据。"""
...
DEFAULT_IMAGE_DOWNLOAD_MAX_BYTES = 50 * 1024 * 1024
...
def _invoke_image_cancel_callback(callback: Callable[[], None] | None) -> None:
    if not callable(callback):
        return
    try:
        callback()
    except ImageOperationCancelled:
        raise
    except Exception as exc:
        raise ImageOperationCancelled(str(exc) or "image task was canceled") from exc
...
def _image_download_max_bytes() -> int:
    return env_int("GPTIMAGE2API_IMAGE_DOWNLOAD_MAX_BYTES", DEFAULT_IMAGE_DOWNLOAD_MAX_BYTES, 1)
...
class OpenAIBackendAPI:
    ...
    def __init__(
        ...
        if self.proxy_profile.clearance_enabled:
            ...
            self.session.headers.update(proxy_settings.build_headers(
    ...
    def _build_fp(self) -> Dict[str, str]:
        ...
        raw_values = (
            {str(k).lower(): str(v) for k, v in raw_fp.items()}
            if isinstance(raw_fp, dict)
            else {}
        )

        def preserved_identifier(key: str) -> str:
            return str(raw_values.get(key) or account.get(key) or "").strip()

        return {
            # Existing device/session IDs remain stable across token refreshes,
            # while every browser-identifying field is forced to Chrome146.
            "user-agent": CHROME146_USER_AGENT,
            "impersonate": CHROME146_IMPERSONATE,
            "oai-device-id": preserved_identifier("oai-device-id") or new_uuid(),
            "oai-session-id": preserved_identifier("oai-session-id") or new_uuid(),
    ...
    @staticmethod
    def _signed_asset_headers(extra: Optional[Dict[str, str]] = None) -> Dict[str, object]:
        return chrome146_signed_asset_headers(extra)
        ...
        for item in limits_progress:
            ...
            if feature in {"image_gen", "image_generation", "image", "images"}:
                try:
                    quota = max(
                        0,
                        int(float(str(item.get("remaining") or item.get("quota") or 0).strip())),
                    )
                except (TypeError, ValueError):
                    quota = 0
                reset_at = str(
    ...
    def _get_default_account(self) -> Dict[str, Any]:
        ...
        response = self.session.get(
            self.base_url + path + f"?timezone_offset_min={CHROME146_TIMEZONE_OFFSET_MIN}",
    ...
    def _codex_responses_headers(self, path: str | None = None) -> Dict[str, str]:
        return self._headers(
            path or "/backend-api/codex/responses",
            {
                "Accept": "text/event-stream",
                "Content-Type": "application/json",
            },
    ...
    def _iter_codex_response_events(raw: Any, max_duration_secs: float | None = None) -> Iterator[Dict[str, Any]]:
        ...
        def _record_event(event: Dict[str, Any]) -> None:
            ...
            lengths = OpenAIBackendAPI._codex_event_image_result_lengths(event)
            if lengths:
                image_result_lengths.extend(lengths)
        ...
        def _flush_sse_event(lines: list[str]) -> tuple[Dict[str, Any] | None, bool]:
            if not lines:
                return None, False
            payload_text = "\n".join(lines).strip()
            lines.clear()
            ...
            if isinstance(data, dict):
                return data, str(data.get("type") or "") in {
                    "response.completed",
                    "response.failed",
                    "response.incomplete",
                }
        ...
        try:
            if "application/json" in content_type:
                ...
                body = read() if callable(read) else bytes(getattr(raw, "content", b"") or b"")
                ...
                lines: list[str] = []
                readline = getattr(raw, "readline", None)
                line_iterator = None
                if not callable(readline):
                    iter_lines = getattr(raw, "iter_lines", None)
                    if not callable(iter_lines):
                        raise TypeError("codex response does not provide a streaming line reader")
                    line_iterator = iter(iter_lines())
                while True:
                    _raise_if_timeout()
                    ...
                    _append_body(line + "\n")
                    if not line:
                        event, terminal = _flush_sse_event(lines)
                ...
                event, _terminal = _flush_sse_event(lines)
                if event is not None:
                    _record_event(event)
                    yield event
                _raise_if_timeout()
        ...
        logger.info({
            ...
            "event_previews": [] if image_result_lengths else event_previews,
            "body_preview": "".join(body_parts)[:1000] if not event_count else "",
        })
    ...
    def iter_codex_image_response_events(
        ...
        stream_timeout = self._deadline_timeout(config.image_stream_timeout_secs)
        ...
        logger.info({
            ...
            "headers": {
                key: value for key, value in self._codex_responses_headers(path).items()
        ...
        try:
            response = self.session.post(
                self.base_url + path,
                headers=self._codex_responses_headers(path),
                json=payload,
            ...
            if status_code >= 400:
                ...
                body_bytes = read() if callable(read) else bytes(getattr(response, "content", b"") or b"")
                body_text = bytes(body_bytes or b"").decode("utf-8", "replace")
                body: Any = body_text
                try:
            ...
            error_text = diagnostic_excerpt(exc, 500)
            ...
            if isinstance(exc, (TimeoutError,)) or "timeout" in error_name:
                raise CodexTransportTimeoutError(
                    f"codex responses transport timeout: {error_text}"
                ) from exc
            if isinstance(exc, (ConnectionError,)) or any(
                marker in error_name
                for marker in ("connection", "proxy", "ssl", "request")
            ):
                raise CodexTransportError(
                    f"codex responses transport failed: {error_text}"
                ) from exc
            raise
        finally:
    ...
    def _prepare_image_conversation(self, prompt: str, requirements: ChatRequirements, model: str) -> str:
        ...
        response = self.session.post(
            ...
            json=payload,
            timeout=self._deadline_timeout(60),
        )
    ...
    def _start_image_generation(self, prompt: str, requirements: ChatRequirements, conduit_token: str, model: str,
        ...
        # diagnostics.
        transport_timeout_secs = self._deadline_timeout(
    ...
    def _get_conversation(self, conversation_id: str, timeout_secs: float = 60) -> Dict[str, Any]:
        ...
        response = self.session.get(self.base_url + path, headers=self._headers(path, {"Accept": "application/json"}),
                                    timeout=self._deadline_timeout(timeout_secs))
        ensure_ok(response, path)
        return response.json()
    ...
    def _resolve_editable_download_url(
        ...
        if artifact.sandbox_path and artifact.message_id:
            ...
            response = self.session.get(
                self.base_url + path,
                headers=self._editable_download_headers(path, conversation_id),
    ...
    def _editable_download_headers(self, path: str, conversation_id: str) -> Dict[str, str]:
        headers = self._editable_browser_headers(path, conversation_id)
        headers["X-OpenAI-Target-Route"] = path
        return headers
    ...
    def _poll_image_results(
        ...
        """
        timeout_secs = self._deadline_timeout(timeout_secs)
        ...
        def _sleep_with_cancellation(seconds: float) -> None:
            ...
            while remaining > 0:
                ...
                self._sleep_for_image_poll(chunk)
                remaining -= chunk
            _invoke_image_cancel_callback(cancel_callback)
    ...
    def _resolve_image_urls_with_timing(
        ...
        started = time.perf_counter()
        try:
            return self._resolve_image_urls(
                conversation_id,
                file_ids,
                sediment_ids,
    ...
    def resolve_conversation_image_urls(
        ...
        timeout = self._deadline_timeout(timeout)
        ...
        if poll and conversation_id and (file_ids or sediment_ids):
            if not config.image_check_before_hit_enabled and not config.image_settle_enabled:
                ...
                if callable(cancel_callback):
                    return self._resolve_image_urls_with_timing(
                        conversation_id,
                        file_ids,
                        sediment_ids,
                        cancel_callback,
    ...
    def download_image_bytes(
        ...
        base_url = str(self.base_url or "").strip() or "https://chatgpt.com"
        for url in urls:
            _invoke_image_cancel_callback(cancel_callback)
            request_url = urljoin(base_url, str(url or "").strip())
            parsed_url = urlparse(request_url)
            same_origin = normalize_url_origin(request_url) == normalize_url_origin(base_url)
            download_headers = (
            ...
            download_headers = proxy_settings.build_headers(
                (
                    chrome146_signed_asset_headers(download_headers)
                    if not same_origin
                    else chrome146_headers(download_headers)
                ),
                ...
                try:
                    if same_origin:
                        ...
                        response = open_verified_returned_image_url(
                            request_url,
                            headers=download_headers,
                            timeout_seconds=self._deadline_timeout(120),
                            max_bytes=max_bytes,
                            proxy=proxy_url,
                    ...
                    break
                except ReturnedUrlVerificationError:
                    raise
                except ImageOperationCancelled:
                    raise
    ...
    def _stream_picture_conversation(
        ...
        try:
            for payload in self._iter_timed_sse_payloads(
                response,
                max_duration_secs=self._deadline_timeout(
                    config.image_stream_timeout_secs
                ),
    ...
    def _chat_target(self) -> tuple[str, str]:
        if self.access_token:
            return "/backend-api/conversation", CHROME146_TIMEZONE
        return "/backend-anon/conversation", CHROME146_TIMEZONE
