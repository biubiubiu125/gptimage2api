...
def _collect_task_ids(value: object) -> list[str]:
    ...
    if isinstance(value, dict):
        for key, item in value.items():
            if key in {"task_id", "image_task_id"} and isinstance(item, str) and item.strip():
                ids.append(item.strip())
            else:
...
def _image_exception_log_status(exc: Exception) -> tuple[str, str]:
    if _final_image_failure(exc).code == "image_task_pending":
        return "任务进行中", "pending"
    return "调用失败", "failed"
...
class LoggedCall:
    ...
    async def run(self, handler, *args, sse: str = "openai", async_before=None):
        ...
        try:
            if async_before is not None:
                ...
                if inspect.isawaitable(prepared):
                    await prepared
                if trace_perf:
                    self.perf_timings["async_before_ms"] = int((time.perf_counter() - prepare_started) * 1000)
        ...
        if isinstance(result, dict):
            ...
            if projected_status not in {"success", "partial_success", "failed", "text_review"}:
    ...
    def stream(self, items):
        ...
        try:
            for item in items:
                if isinstance(item, dict):
                    item_status = str(item.get("_call_status") or "").strip().lower()
                    if item_status == "failed":
                        projected_status = "failed"
                    elif item_status == "text_review" and projected_status != "failed":
                        projected_status = "text_review"
                    elif item_status == "partial_success" and projected_status == "success":
                        projected_status = "partial_success"
                urls.extend(_collect_urls(item))
