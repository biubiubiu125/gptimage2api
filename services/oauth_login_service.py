...
得到的 refresh_token 跟 account_service 刷新 AT 使用的 client_id 是同一个
（app_2SKx67EdpoN0G6j64rFvigXD），落盘后可用于手动刷新或 AT 临期自动续期。
"""
...
class OAuthLoginService:
    ...
    def _exchange_code(code: str, code_verifier: str, redirect_uri: str) -> dict[str, str]:
        ...
        if response.status_code != 200 or not isinstance(data, dict) or not data.get("access_token"):
            ...
            if not detail:
                try:
                    detail = sanitize_diagnostic_text(response.text, limit=300)
                except Exception:
                    detail = ""
