...
class _ImageStorageFields(_StrictModel):
    ...
    webdav_username: str = ""
    webdav_password: str = ""
    webdav_root_path: str = "gptimage2api/images"
    public_base_url: str = ""
...
class BackupIncludeSettings(_StrictModel):
    ...
    images: bool = False
    image_queue: bool = True
    registration: bool = Field(default=True, alias="register")
...
class _ProxyRuntimeClearanceFields(_StrictModel):
    ...
    @classmethod
    def _drop_legacy_flaresolverr(cls, value: object) -> object:
        if not isinstance(value, dict):
            return value
        data = dict(value)
        data.pop("flaresolverr_url", None)
        if str(data.get("mode") or "").strip().lower() == "flaresolverr":
            data["mode"] = "none"
        return data
