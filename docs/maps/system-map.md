...
flowchart LR
    ...
    Browser["Vue control panel"] --> ControlAPI["Control API<br/>api/accounts.py · api/system.py · api/image_tasks.py · api/register.py · api/prompts.py"]
    ...
    Domain --> QueueDB["Image Queue Store<br/>PostgreSQL"]
...
| Product navigation, charts, workflows, and responsive composition | `gptimage2api/web-vue` | Control-panel users |
...
| Settings, Proxy Groups, Account Groups, remote-import configuration, registration configuration/runtime, Call Records, dashboard aggregates, Prompt Library snapshots, coordination state, and Editable File Tasks | Separate repositories in the Application Database | Their domain services and repository interfaces |
| Image Tasks, attempts, leases, retries, and terminal references | `gptimage2api_image_queue` Image Queue Store | `ImageTaskService` and `services/image_queue/` |
...
| Registration execution and handoff | `RegisterService` with `yyds_mail`, `remail`, `outlook_token`, and `icloud_api` | Registration API, console, bounded pending-result reconciliation, encrypted config store, and Account Repository |
| Studio conversation presentation state | Browser-local storage | Studio conversation runtimes |

Backup restore is a maintenance operation: `BackupService` serializes restore
requests, stops registration and image queue writers through App lifecycle hooks,
and requires a process restart before restored repository state is used.

One physical Application Database does not create one generic repository or one
transaction boundary. Image Assets, Image Queue state, gallery files, and live
