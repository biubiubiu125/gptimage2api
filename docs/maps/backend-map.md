...
[`../../api/app.py`](../../api/app.py) assembles six routers:
...
| [`api/register.py`](../../api/register.py) | Admin registration configuration, status, and task actions | `RegisterService`, registration repositories |
...
| Image Tasks | `ImageTaskService` | Owner-scoped task projection and terminal result | `gptimage2api_image_queue` Image Queue Store |
...
| Registration | `RegisterService`, `services/register/` | Four-provider registration projection, task actions, and pending-result reconciliation | Registration configuration/runtime repositories, pending handoff file, plus Account Repository |
...
| `services/file_lock.py` | Coordinates file-backed mutations | Used by registration state and other file-backed owners |
...
Codex image requests are sent through the `OpenAIBackendAPI` configured
`curl_cffi` session. The Codex Responses path inherits the selected proxy,
image egress reservation, browser headers, target route, timeout, and
account-attributed transport classification; it is not a separate direct
transport.
