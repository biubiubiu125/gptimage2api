...
`docker-compose.postgres.yml` 使用官方 `postgres:18-alpine` 镜像，等待数据库健康后由一次性 `image-queue-init` 服务幂等创建队列数据库，再启动应用，并将数据持久化到 `gptimage2api-postgres-data` 命名卷。Application Database 固定为 `gptimage2api_app`，Image Queue Store 固定为 `gptimage2api_image_queue`；PostgreSQL 健康检查只负责报告可用性，不执行写入操作。捆绑 PostgreSQL 启动参数含 `max_connections=500`；自动队列连接池每副本最多 80+40，加上应用库 10+20，两副本合计 300，给超级用户和健康检查留余量。应用容器 `nofile` 为 262144，`nproc` 为 65535，`stop_grace_period` 为 300 秒，让默认生图（SSE 80 秒 + 轮询 60 秒）能在停机宽限内画完。句柄硬墙按进程 `nofile-1024` 封顶；已在跑、`nofile` 仍是 65535 的容器需要重建后才会落到该上限。同宿主机多副本不要无 CPU 限额叠加；每份都会按整机核数计算自动生图上限，不要在 Compose 里写死 `cpus:`。超时停机不会立刻放掉 PostgreSQL 账号租约；未完成领取继续占槽并续租，等执行线程结束或进程被杀掉。保存和变换阶段会先放掉账号租约，本机占用不再把这些任务算进账号槽；本机生图名额按领取时阶段计算，下载过程仍占着生图槽。备份恢复先停图片队列，未排空会立刻中止并拉回已停服务；其它副本只要心跳仍新，或本机还有未完成领取 / 调度线程，都会拒绝恢复。恢复进行中除 `/health` 外 HTTP 返回 503。任何 `pg_restore --clean` 之前都会先检查队列并关掉连接。含队列的备份先恢复队列 dump，再覆盖应用库。应用库 `pg_restore --clean` 前会先关掉账号连接池；恢复期间 `/health` 不再查库，避免和 ACCESS EXCLUSIVE 顶牛。只有恢复成功才会把图片队列、注册、GenBox、备份调度和后台线程拉起来，失败保持停机；HTTP 入队在队列未启动时拒绝。恢复后会收掉快照里未过期的在画租约，并把旧工作进程心跳标过期。HTTP 入队不再跟 85% 连接池硬墙，新生图仍看连接池占用。监控占用门会推进 2.5 秒计时，积压满或积压查询失败时关门。外接数据库请保证连接数不低于 500。数据库端口默认不暴露到宿主机。应用、PostgreSQL 和队列初始化都加入独立网 `gptimage2api-network`，不和其他 Compose 项目共用默认桥接。
...
一键安装向导固定使用 PostgreSQL 18 本地容器，不再询问 SQLite 或外部数据库 URL，也不再询问 Git 分支。直接回车使用默认值，并立刻打印已选项。向导会询问图片访问地址并写入 `GPTIMAGE2API_BASE_URL`，默认 `http://localhost:<端口>`；外网请改成客户端能打开的域名或 IP。管理员登录密钥必须手工输入两次，输入过程隐藏。摘要确认后才开始拉镜像或克隆。Docker 模式启动应用与 PostgreSQL；Python 源码模式仍会启动 PostgreSQL 容器，并把 `127.0.0.1:5432` 映射到宿主机，不对外网开放。若安装目录已是旧 Git 仓库且无法快进到新的 `main`，需要备份后删除该目录再装。重复运行会复用已有 `POSTGRES_PASSWORD`。向导不再询问后端同步并发容量，默认写入 `GPTIMAGE2API_THREAD_TOKENS=0` 和 `GPTIMAGE2API_IMAGE_QUEUE_GENERATION_CONCURRENCY=0`（自动：生图上限按核数，门口令牌与生图上限相同）。队列积压上限默认 `GPTIMAGE2API_IMAGE_QUEUE_MAX_BACKLOG=50`。占用门默认 CPU/内存/Swap ≥90% 持续 2.5 秒关门、三项都 <80% 持续 2.5 秒开门，可用 `GPTIMAGE2API_IMAGE_QUEUE_OCCUPANCY_PAUSE_PERCENT` / `RESUME_PERCENT` / `HOLD_SECONDS` 覆盖；线程硬墙 `GPTIMAGE2API_IMAGE_QUEUE_ABSOLUTE_GUARD=0` 表示自动。已在跑的 `.env` 不会自动改写。
