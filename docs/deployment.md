...
发布镜像为 `ghcr.io/biubiubiu125/gptimage2api`。标准 Compose 将服务暴露在 `2080` 端口，使用
...
`docker-compose.postgres.yml` 使用官方 `postgres:18-alpine` 镜像，等待数据库健康后由一次性 `image-queue-init` 服务幂等创建队列数据库，再启动应用，并将数据持久化到 `gptimage2api-postgres-data` 命名卷。Application Database 固定为 `gptimage2api_app`，Image Queue Store 固定为 `gptimage2api_image_queue`；PostgreSQL 健康检查只负责报告可用性，不执行写入操作。捆绑 PostgreSQL 启动参数含 `max_connections=500`；自动队列连接池每副本最多 80+40，加上应用库 10+20，两副本合计 300，给超级用户和健康检查留余量。应用容器 `nofile` 为 262144，`nproc` 为 65535，`stop_grace_period` 为 300 秒，让默认生图（SSE 80 秒 + 轮询 60 秒）能在停机宽限内画完。句柄硬墙按进程 `nofile-1024` 封顶；已在跑、`nofile` 仍是 65535 的容器需要重建后才会落到该上限。同宿主机多副本不要无 CPU 限额叠加。核数优先读容器 cgroup CPU 限额，没有限额再用整机核数；不要在 Compose 里写死 `cpus:`，否则自动生图上限会按限额算。超时停机不会立刻放掉 PostgreSQL 账号租约；未完成领取继续占槽并续租，等执行线程结束或进程被杀掉。未排空停机即使快照里 `worker_active=False`，心跳仍新时其它副本也不会抢走租约。保存和变换阶段会先放掉账号租约，本机占用不再把这些任务算进账号槽；本机生图名额按领取时阶段计算，下载过程仍占着生图槽。备份恢复先停图片队列，未排空会立刻中止并拉回已停服务；其它副本只要心跳仍新，或本机还有未完成领取 / 调度线程，都会拒绝恢复。恢复进行中除 `/health` 外 HTTP 返回 503。任何 `pg_restore --clean` 之前都会先检查队列并关掉连接。含队列的备份先恢复队列 dump，再覆盖应用库。应用库 `pg_restore --clean --single-transaction` 前会先关掉账号连接池；失败整段回滚，避免半截账号库。恢复期间 `/health` 不再查库，避免和 ACCESS EXCLUSIVE 顶牛。只有恢复成功才会把图片队列、注册、GenBox、备份调度和后台线程拉起来，失败保持停机；队列没起来就不拉后面的服务；HTTP 入队在队列未启动、调度被打死或调度线程已停时拒绝。队列 dump 写入后会落下本地回收标记，后续普通启动也会按「刚恢复过」收掉快照里未过期的在画租约。同进程第二次恢复仍会再停队列；调度被打死或队列库已关掉且没有在手任务时，`start()` 会重新进恢复。身份冲突时还在画的任务继续心跳续租；占用放回失败会失败任务，两次都失败则继续续租。恢复后会收掉快照里未过期的在画租约，并把旧工作进程心跳标过期、清空 `process_instance_id`。活工人先看心跳，心跳过期后仍看未过期作业租约，租约过期但仍在最长领取时间内心跳的在画任务也算活着，监控 workers 列表会保留未排空但仍在心跳的副本。恢复中止不会把身份冲突的调度拉起来。领取阶段身份冲突同样置 fatal。监控账号空位探测失败保留上次快照，「当前在画」读本机实际领取。备份恢复先停注册并立刻打停机事件，恢复期间不再继续自动注册；注册关机超时会中止恢复，不再继续清库。占用门闩来源不写注册探测，生图或入队探测会改成对应来源；监控不再显示注册探测关门。占用门只认 `OCCUPANCY_*`。心跳线程已停但调度线程仍活时，调度循环会把心跳拉起来继续续租；健康检查仍按不可用，占用门保持现场，HTTP 入队不因此拒绝。残留心跳还有在手任务、心跳却已停时，健康检查、监控和入队也会把心跳拉起来续租，但仍拒绝新入队；在手任务画完后残留心跳自己退出，没有在手任务后下一次入队再把队列拉起来。不健康队列重置时 `stop()` 抛错不当成已排空；残留心跳 join 超时拒绝再开新心跳，排空成功也要心跳真正停掉才标停机，重置按心跳间隔等待。领取前占用复查采现场，不再把入队时的 T0 快照写回占用门闩。HTTP 入队不再跟 85% 连接池硬墙，新生图仍看连接池占用。监控占用门会推进 2.5 秒计时，积压满或积压查询失败时关门。外接数据库请保证连接数不低于 500。数据库端口默认不暴露到宿主机。应用、PostgreSQL 和队列初始化都加入独立网 `gptimage2api-network`，不和其他 Compose 项目共用默认桥接。
...
一键安装向导固定使用 PostgreSQL 18 本地容器，不再询问 SQLite 或外部数据库 URL，也不再询问 Git 分支。直接回车使用默认值，并立刻打印已选项。向导会询问图片访问地址并写入 `GPTIMAGE2API_BASE_URL`，默认 `http://localhost:<端口>`；外网请改成客户端能打开的域名或 IP。管理员登录密钥必须手工输入两次，输入过程隐藏。摘要确认后才开始拉镜像或克隆。Docker 模式启动应用与 PostgreSQL；Python 源码模式仍会启动 PostgreSQL 容器，并把 `127.0.0.1:5432` 映射到宿主机，不对外网开放。若安装目录已是旧 Git 仓库且无法快进到新的 `main`，需要备份后删除该目录再装。重复运行会复用已有 `POSTGRES_PASSWORD`。向导不再询问后端同步并发容量，默认写入 `GPTIMAGE2API_THREAD_TOKENS=0` 和 `GPTIMAGE2API_IMAGE_QUEUE_GENERATION_CONCURRENCY=0`（自动：生图上限按 cgroup CPU 限额，没有限额再用整机核数，门口令牌与生图上限相同）。队列积压上限默认 `GPTIMAGE2API_IMAGE_QUEUE_MAX_BACKLOG=50`。占用门默认 CPU/内存/Swap ≥90% 持续 2.5 秒关门、三项都 <80% 持续 2.5 秒开门，可用 `GPTIMAGE2API_IMAGE_QUEUE_OCCUPANCY_PAUSE_PERCENT` / `RESUME_PERCENT` / `HOLD_SECONDS` 覆盖；线程硬墙 `GPTIMAGE2API_IMAGE_QUEUE_ABSOLUTE_GUARD=0` 表示自动。已在跑的 `.env` 不会自动改写。
