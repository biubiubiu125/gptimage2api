...
GPTImage2API exposes OpenAI-compatible image interfaces backed
by an Account Pool of Upstream Accounts and retains the upstream text/search compatibility surface. The control panel observes and manages
...
| `/register` | Manage the four-provider registration machine and mailbox pools |
...
There is no current documentation-center route.
...
- Real-time observations, Image Assets, gallery files, and Editable
  File Assets remain outside the Application Database and retain their own
  lifecycle owners. Durable Image Tasks are stored in the dedicated Image Queue Store.
...
theme states. `gptimage2api` owns AppShell, page composition, tables, charts,
