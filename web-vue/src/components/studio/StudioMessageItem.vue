          ...
          >
            ...
            <template v-else>
              <div
                ...
                :class="{ 'is-error': message.task?.status === 'failed' || message.task?.status === 'cancelled' }"
              >
                <Icon
                  :icon="message.task?.status === 'cancelled'
                      ? 'lucide:ban'
                      : 'lucide:circle-alert'"
                  class="h-4 w-4"
                />
...
export type StudioMessageActionKey = 'copy' | 'edit' | 'resend' | 'fill' | 'resume-poll' | 'cancel-image-task' | 'retry' | 'delete'
...
function actionsForMessage(message: StudioMessageView): StudioMessageAction[] {
  ...
  if (message.role === 'user') {
    ...
    message.mode === 'image'
    && message.task?.actions.cancel
  ) {
    actions.push({ key: 'cancel-image-task', label: '取消任务', icon: 'lucide:ban', danger: true })
  } else if (
    message.mode === 'image'
