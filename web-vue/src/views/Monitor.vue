      ...
      <PanelHeader title="实时监控" align="start">
        <template #copy>
          <p class="mt-1 text-xs text-muted-foreground">
            进程内实时窗口，用于观察入口、账号、出口、上游生成、断流和本地拒绝/繁忙；入口排队高时关注 GPTIMAGE2API_THREAD_TOKENS；最近更新：{{ monitorData?.updated_at || '未获取' }}
          </p>
        </template>
        <template #actions>
...
const REFRESH_INTERVAL_STORAGE_KEY = 'gptimage2api_monitor_refresh_interval_secs'
