...
export function useSettingsBackupRuntime(options: SettingsBackupRuntimeOptions) {
  ...
  async function restoreBackupItem(item: BackupItem) {
    ...
    await progressRuntime.start({
      title: '恢复备份',
      subtitle: item.name || item.key,
      message: '正在下载、校验并恢复备份内容...',
    })
    try {
      const response = await settingsApi.restoreBackup(item.key)
      const restored = Object.entries(response.result.restored || {})
        .filter(([, included]) => included)
        .map(([name]) => name)
        .join('、')
      progressRuntime.succeed(
        restored
          ? `备份已恢复（${restored}），请重启服务完成切换`
          : '备份已恢复，请重启服务完成切换',
      )
    } catch (error) {
      progressRuntime.fail(errorMessage(error, '恢复备份失败'))
    } finally {
      backupBusy.value = ''
    }
  }
