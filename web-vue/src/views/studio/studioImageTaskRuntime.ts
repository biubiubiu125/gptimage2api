...
export function useStudioImageTaskRuntime(input: StudioImageTaskRuntimeInput) {
  ...
  const storedTaskIdsVersion = ref(0)
  ...
  function rememberTask(taskId: string) {
    ...
    storedTaskIdsVersion.value += 1
  }

  function forgetTasks(taskIds: string[]) {
    const remove = new Set(taskIds.map((id) => cleanStudioText(id)).filter(Boolean))
    if (!remove.size) return
    const retained = storedImageTaskIds().filter((id) => !remove.has(id))
    setJsonPreference(preferenceKeys.imageTaskLocalIds, retained)
    storedTaskIdsVersion.value += 1
  }

  const requestedImageTaskIds = computed(() => {
    void storedTaskIdsVersion.value
  ...
  function syncMessageStatuses() {
    ...
    input.conversationRuntimeIndex.value.imageTaskMessageEntries.forEach(({ conversation, message }) => {
      ...
      } else if (task.status === 'succeeded' || task.status === 'partial_success') {
        message.status = 'done'
        message.error = undefined
        if (previousStatus !== 'done') input.hooks.markConversationNotice(conversation.id, 'done')
      } else if (task.status === 'cancelled') {
        message.status = 'error'
        message.error = task.public_error || '图片任务已取消'
  ...
  async function cancelTask(taskId: string) {
    ...
    if (!normalizedTaskId) return null
    try {
      const task = await imageTasksApi.cancel(normalizedTaskId)
      merge([task])
      syncMessageStatuses()
      taskPollingCoordinator.schedulePoll()
      return task
