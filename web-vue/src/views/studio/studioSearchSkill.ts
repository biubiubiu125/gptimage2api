...
export function buildSearchSkillDocument(input: SearchSkillPromptInput) {
  ...
  if (input.language === 'en') {
    return `---
name: gptimage2api-search
description: Use when current web search is needed through this gptimage2api server. Call the configured HTTP search endpoint with a prompt and return the answer with source URLs.
---
...
Authorization: Bearer \${GPTIMAGE2API_API_KEY}
...
- Read the API key from the \`GPTIMAGE2API_API_KEY\` environment variable at request time.
...
- PowerShell: \`$env:GPTIMAGE2API_API_KEY = '<your-api-key>'\`
- Bash / zsh: \`export GPTIMAGE2API_API_KEY='<your-api-key>'\``
  }

  return `---
name: gptimage2api-search
description: 当用户需要联网搜索、查询最新信息、核实事实或需要来源链接时，调用本地 gptimage2api 搜索接口。
---
...
- 发起请求时从 \`GPTIMAGE2API_API_KEY\` 环境变量读取 API Key。
...
2. Skill name: gptimage2api-search
3. File name: SKILL.md
4. Only create or update this skill file.
5. Do not write a real API key into the skill. It must read \`GPTIMAGE2API_API_KEY\` from the environment at runtime.
...
2. skill 名称为：gptimage2api-search
3. 文件名为：SKILL.md
4. 只创建或更新这个 skill 文件，不要修改其他无关文件。
5. 不要把真实 API Key 写入 skill，运行时必须从环境变量 \`GPTIMAGE2API_API_KEY\` 读取。
