...
export interface BackupRestoreResult {
  ...
  restored: {
    application_database: boolean
    image_tasks: boolean
    editable_files: boolean
    images: boolean
    image_queue: boolean
...
export const PUBLIC_SETTINGS_CHANGED_EVENT = 'gptimage2api:public-settings-changed'
...
export const settingsApi = {
  ...
  restoreBackup: (key: string) =>
    apiClient.post<{ key: string; confirm: boolean }, { result: BackupRestoreResult }>(
      '/api/backups/restore',
      { key, confirm: true },
    ),
