...
export type ImageTaskStatus = 'pending' | 'running' | 'retrying' | 'succeeded' | 'partial_success' | 'failed' | 'cancelled'
...
function parseImageTask(value: unknown, path = 'response'): ImageTask {
  ...
  const expectedTerminal = status !== 'pending' && status !== 'running' && status !== 'retrying'
  ...
  return {
    ...
    actions: {
      ...
      cancel: expectBoolean(actions.cancel, `${path}.actions.cancel`),
    },
  }
}
...
function localImageUrl(path: string) {
  const cleaned = cleanString(path).replace(/^\/+/, '')
  if (!cleaned) return ''
...
export function imageAssetUrl(asset: ImageTaskAsset) {
  ...
  return localImageUrl(asset.path)
}
