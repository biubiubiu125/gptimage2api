...
def _source_request_marker(source: ImageSource) -> dict[str, object] | None:
    if isinstance(source, str):
        return {
            "kind": "reference",
            "sha256": hashlib.sha256(source.strip().encode("utf-8")).hexdigest(),
        }
...
def image_edit_source_request_hash(
    ...
    markers = [
        _source_request_marker(source)
        for source in [*image_sources, *mask_sources]
    ]
    ...
    fingerprint = {
        ...
        "image_sources": markers[:image_count],
        "mask_sources": markers[image_count:],
    }
    encoded = json.dumps(
        fingerprint,
        ensure_ascii=False,
...
def _decode_data_url(
    url: str,
    *,
    max_bytes: int = MAX_IMAGE_REFERENCE_BYTES,
) -> ImageInput:
...
def _download_image_url(
    ...
    *,
    max_bytes: int = MAX_IMAGE_REFERENCE_BYTES,
    user_agent: str = CHROME146_USER_AGENT,
) -> ImageInput:
    """下载远程图片：把 http/https 图片链接转成标准图片输入元组。"""
    limit = max(1, int(max_bytes))
    source = _clean(url)
    if source.startswith("data:"):
        return _decode_data_url(source, max_bytes=limit)
    current = source
    for redirect_count in range(MAX_IMAGE_REDIRECTS + 1):
        ...
        try:
            ...
            request_headers = proxy_settings.build_headers(
                chrome146_headers({
                    "Accept": "image/*,*/*;q=0.8",
