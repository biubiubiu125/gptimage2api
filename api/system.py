...
def _image_maintenance_http_exception(exc: Exception) -> HTTPException:
...
def _public_health_component(value: object, *, fallback_error: str) -> dict[str, object]:
...
def create_router(app_version: str) -> APIRouter:
    ...
    async def health():
        ...
        try:
            image_queue = await run_in_threadpool(image_task_service.health_summary)
        except Exception as exc:
            logger.error({
        ...
        healthy = (
            ...
            and public_image_queue.get("healthy") is True
        )
        return JSONResponse(
            ...
            content={
                ...
                "healthy": healthy,
                "version": app_version,
                "application_database": public_application_database,
                "image_queue": public_image_queue,
            },
        )
    ...
    async def cleanup_expired_images_endpoint(authorization: str | None = Header(default=None)):
        ...
        except (*_IMAGE_QUEUE_ERRORS, ImageStorageError) as exc:
            raise _image_maintenance_http_exception(exc) from exc
    ...
    async def restore_backup_endpoint(body: BackupRestoreRequest, authorization: str | None = Header(default=None)):
        require_admin(authorization)
        if not body.confirm:
            raise HTTPException(status_code=400, detail={"error": "恢复备份必须显式确认"})
        try:
            return {
                "result": await run_in_threadpool(backup_service.restore_backup, body.key),
            }
        except BackupError as exc:
            raise HTTPException(status_code=400, detail={"error": str(exc)}) from exc
