# GPTImage2API Context

GPTImage2API exposes OpenAI-compatible image interfaces backed by a managed pool of upstream ChatGPT accounts and the retained upstream text/search compatibility surface. Its control panel observes and manages those calls without becoming a second source of business semantics.
...
**Image Queue Store**:
The dedicated PostgreSQL database that persists Image Tasks, Image Jobs, Image Task Events, leases, artifacts, and worker heartbeats for the in-process image queue. It is required for image generation durability and is separate from the Application Database.
_Avoid_: Application Database, image file storage, live browser state
...
- An **Image Task** is persisted in the **Image Queue Store**, produces zero or more **Image Assets**, and exposes one terminal result.
...
- **Image Assets**, gallery metadata, and live monitoring state do not belong to the **Application Database**.
- The **Image Queue Store** owns durable image queue state and remains independent from registration scheduling.
