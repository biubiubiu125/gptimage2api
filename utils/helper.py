...
MAX_JSON_IMAGE_INPUTS = 16
MAX_JSON_IMAGE_TOTAL_BYTES = 100 * 1024 * 1024
...
def _decode_message_image_url(value: object) -> tuple[bytes, str] | None:
    ...
    try:
        # Chat/Responses image inputs must share the multipart endpoint's
        # public-URL, redirect, and bounded-body checks.  Import lazily to
        # avoid making the low-level helper depend on the API package at
        # module-import time.
        from api.image_inputs import _download_image_url

        image_data, _filename, mime = _download_image_url(
            source,
